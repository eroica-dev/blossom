...
impl NodeRuntime {
    ...
    pub fn install_membership_lease(
        ...
        let relays = self.verified_relay_set(&members);
        let view = Arc::new(VerifiedMembershipView {
            group_id: self.inner.group_id,
            epoch_hash,
            epoch_nonce: epoch_nonce.0,
            valid_until: std::time::Instant::now()
                + std::time::Duration::from_millis(certificate.statement.valid_for_millis),
    ...
    fn verified_relay_set(&self, members: &crate::membership::MemberSet) -> RelaySet {
        self.inner
            .address_book
            .read()
            .expect("address book lock poisoned")
            .active_signed_services(self.inner.group_id, service_unix_time_millis())
            .filter(|service| {
                service.kind == ServiceKind::Relay
                    && members.get(&service.public_key).is_some_and(|member| {
    ...
    fn republish_fresh_membership_relays(&self, members: &crate::membership::MemberSet) {
    ...
    pub fn try_produce_commit(&self, round: u8) -> Result<Option<Commit>> {
        ...
        let mut chain_extended = false;
        {
            ...
            let quorum = state.get_mut_quorum(&target.last_epoch, target.nonce, round);
            record_commit_vote(quorum, message.clone());
...
mod tests {
    ...
    fn verified_membership_watch_is_quorum_renewed_and_fail_closed() {
        ...
        let votes = keypairs[..4]
            .iter()
            .map(|keypair| {
                MembershipLeaseVote::signed(statement.clone(), &keypair.signer()).unwrap()
            })
            .collect::<Vec<_>>();
        ...
        let record = SignedServiceRecord::signed(
            crate::address_book::ServiceRecordBody {
                ...
                port: 7443,
                generation: 1,
                expires_at_unix_millis: service_unix_time_millis() + 10_000,
                tombstone: false,
            },
            &relay_owner.signer(),
        )
