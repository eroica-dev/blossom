...
mod tests {
    ...
    fn public_secret_and_signature_hex_round_trip() {
        ...
        let secret_hex = hex::encode(keypair.secret.as_array());
        ...
        assert_eq!(
            SecKey::try_from_hex(&secret_hex).unwrap().as_array(),
            keypair.secret.as_array()
        );
        assert_eq!(
    ...
    #[test]
    fn public_material_serde_round_trip_uses_hex_strings() {
        let keypair = Keypair::generate();
        let signature = Signature::sign(b"message", &keypair.secret);
        ...
        assert!(format!("{:?}", keypair.secret).contains("[REDACTED]"));
    }
}
