...
impl ServiceRegistration {
    pub fn service(&self) -> &Service {
        match self {
            ...
            Self::NodeAdmission(admission) => &admission.body.service,
        }
    }

    pub fn into_parts(self) -> (Service, Option<NodeAdmission>) {
        match self {
            Self::Service(service) => (service, None),
            Self::NodeAdmission(admission) => {
                let admission = *admission;
                (admission.body.service.clone(), Some(admission))
            }
        }
    }
}

impl From<Service> for ServiceRegistration {
