#!/usr/bin/env bash
# Run deterministic node I/O fuzzing through the blossom-lab crate.
#
# Environment:
#   NODES=1
#   CASES=256
...
out="$root/results/lab_fuzz_$(timestamp).csv"
...
pinned_exec cargo run --release -p blossom-lab --bin blossom-lab-fuzz -- \
  --nodes "${NODES:-1}" \
  --cases "${CASES:-256}" \
