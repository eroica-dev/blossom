#!/usr/bin/env bash
# Build and run the Blossom lab in a VM-like container boundary, with a native
...
#   MODE=chaos REQUESTS=1000 LATENCY_MS=5 ./benchmarks/scripts/run-lab-container.sh
#   LAB_BACKEND=native REQUESTS=100 ./benchmarks/scripts/run-lab-container.sh sim
#   DRY_RUN=1 ./benchmarks/scripts/run-lab-container.sh fuzz
#
# Environment:
#   MODE=sim|chaos|epoch-chaos|fuzz|all
#   LAB_BACKEND=auto|docker|native
#   ALLOW_NATIVE_TCP=0
#   BLOSSOM_LAB_IMAGE=blossom-lab:local
#   BLOSSOM_LAB_FEATURES=filtered-transactions,insecure-fast-hash
#   BUILD=1
#   PULL=0
...
image="${BLOSSOM_LAB_IMAGE:-blossom-lab:local}"
containerfile="${BLOSSOM_LAB_CONTAINERFILE:-$ws_root/crates/blossom-lab/container/Dockerfile}"
runtime_network="${RUNTIME_NETWORK:-none}"
backend="${LAB_BACKEND:-auto}"
...
run_docker_backend() {
  ...
  if [[ -n "${BLOSSOM_LAB_FEATURES:-}" ]]; then
    build_args+=(--build-arg "BLOSSOM_LAB_FEATURES=$BLOSSOM_LAB_FEATURES")
  fi
  build_args+=("$ws_root")
...
run_native_tcp_or_fail() {
  ...
  echo "Set ALLOW_NATIVE_TCP=1 to run the same lab natively without OS containment." >&2
  exit 69
}
...
run_native_backend() {
  case "$mode" in
    ...
    epoch-chaos)
      ...
      ;;
    fuzz)
      run_native_tcp_or_fail fuzz "$here/run-lab-fuzz.sh" "$@"
      ;;
    all)
      echo "using native hermetic backend for sim; no sockets or wall-clock sleeps are used" >&2
      run_cmd "$here/run-lab-sim.sh" "$@"
      ...
      ;;
    *)
      echo "unknown Blossom lab mode: $mode" >&2
...
case "$backend" in
  ...
  auto)
    ...
    ;;
  *)
    echo "unknown LAB_BACKEND=$backend" >&2
