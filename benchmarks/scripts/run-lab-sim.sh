#!/usr/bin/env bash
# Run the hermetic socket-free Blossom lab simulator.
#
# Environment:
#   NODES=6
#   REQUESTS=100
...
summary="$root/results/lab_sim_$(timestamp).csv"
...
pinned_exec cargo run --release -p blossom-lab --bin blossom-lab-sim -- \
  --nodes "${NODES:-6}" \
