...
async fn cluster_exposes_health_state_address_book_and_nonce() {
    ...
    for index in 0..cluster.len() {
        ...
        match node
            .request(WireRequest::EpochChainRange {
                from_nonce: Nonce::new(0),
                max_epochs: 1,
            })
            .await
            .unwrap()
        {
            ...
            response => panic!("expected epoch chain range, got {}", response.kind()),
        }
...
async fn wait_for_same_finalized_epoch(
    ...
    loop {
        ...
        if final_hashes.len() == 1 && block_counts.len() == 1 {
            return Ok((
        ...
        if std::time::Instant::now() >= deadline {
            return Err(blossom::BlossomError::WireProtocol(format!(
                "cluster did not finalize the same epoch before deadline: finalized_nodes={}, unique_hashes={}, block_counts={:?}",
                finalized_nodes,
                final_hashes.len(),
                block_counts
            )));
