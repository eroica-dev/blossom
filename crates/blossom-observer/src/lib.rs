//! Collection and analysis of Blossom JSONL telemetry.
//!
//! [`ObserverCollector`] accepts structured [`blossom::TelemetryEvent`] values
//! or newline-delimited JSON and produces an immutable [`ObserverAnalysis`].
//! Collection is thread-safe so a service can share one collector across
//! multiple telemetry connections.
...
/// Thread-safe in-memory collection of Blossom telemetry events.
...
impl ObserverCollector {
    /// Appends one already-decoded telemetry event.
    ...
    /// Decodes and records one JSONL line.
    ///
    /// Empty or whitespace-only lines are accepted and ignored.
    ...
    /// Returns a point-in-time copy of every collected event.
    ...
    /// Analyzes a point-in-time copy of the collected events.
    pub fn analysis(&self) -> ObserverAnalysis {
        analyze_events(&self.events())
    }
}

/// Aggregate health, traffic, membership, and span statistics.
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Eq, Default)]
pub struct ObserverAnalysis {
    /// Total decoded telemetry events.
    pub total_events: usize,
    /// Number of span-start records.
    pub span_starts: usize,
    /// Number of span-end records.
    pub span_ends: usize,
    /// Number of non-span event records.
    pub event_records: usize,
    /// Number of events carrying an error or error outcome.
    pub error_events: usize,
    /// Number of membership-drop events.
    pub dropped_node_events: usize,
    /// Number of membership-reconnect events.
    pub reconnected_node_events: usize,
    /// Total successful reconnect catch-up proofs reported by events.
    pub reconnect_catchup_proofs: u64,
    /// Total stale reconnect proofs reported by events.
    pub reconnect_stale_proofs: u64,
    /// Total reconnect identity rejections reported by events.
    pub reconnect_identity_rejections: u64,
    /// Total block-transfer attempts reported by events.
    pub block_transfer_attempts: u64,
    /// Total accepted blocks reported by events.
    pub accepted_blocks: u64,
    /// Total denied blocks reported by events.
    pub denied_blocks: u64,
    /// Total accepted block payload bytes.
    pub accepted_block_bytes: u64,
    /// Total denied block payload bytes.
    pub denied_block_bytes: u64,
    /// Number of span starts without a matching end.
    pub incomplete_spans: usize,
    /// Number of span ends without a matching start.
    pub orphan_span_ends: usize,
    /// Number of distinct node identities represented.
    pub nodes: usize,
    /// Event count keyed by stage.
    pub stages: BTreeMap<String, usize>,
    /// Event count keyed by `stage:event`.
    pub events_by_stage: BTreeMap<String, usize>,
    /// Event count keyed by node.
    pub events_by_node: BTreeMap<String, usize>,
    /// Span-start count keyed by node.
    pub span_starts_by_node: BTreeMap<String, usize>,
    /// Span-end count keyed by node.
    pub span_ends_by_node: BTreeMap<String, usize>,
    /// Error count keyed by node.
    pub errors_by_node: BTreeMap<String, usize>,
    /// Membership-drop count keyed by node.
    pub dropped_nodes_by_node: BTreeMap<String, usize>,
    /// Membership-reconnect count keyed by node.
    pub reconnected_nodes_by_node: BTreeMap<String, usize>,
    /// Incomplete span count keyed by node.
    pub incomplete_spans_by_node: BTreeMap<String, usize>,
    /// Orphan span-end count keyed by node.
    pub orphan_span_ends_by_node: BTreeMap<String, usize>,
    /// Latest event timestamp, in microseconds, keyed by node.
    pub last_event_timestamp_micros_by_node: BTreeMap<String, u128>,
    /// Error count keyed by stage.
    pub errors_by_stage: BTreeMap<String, usize>,
    /// Completed span durations keyed by stage.
    pub span_durations_by_stage: BTreeMap<String, SpanDurationStats>,
    /// Completed span durations keyed by `stage:event`.
    pub span_durations_by_stage_event: BTreeMap<String, SpanDurationStats>,
    /// Completed span durations keyed by `node:stage`.
    pub span_durations_by_node_stage: BTreeMap<String, SpanDurationStats>,
}

/// Summary statistics for a set of completed spans.
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Eq, Default)]
pub struct SpanDurationStats {
    ...
    /// Shortest span duration in microseconds.
    pub min_micros: u128,
    /// Longest span duration in microseconds.
    pub max_micros: u128,
    /// Integer mean span duration in microseconds.
    pub avg_micros: u128,
}
...
/// Produces aggregate statistics from an ordered or unordered event slice.
///
/// Spans are matched by node, consensus group, and span identifier.
#[must_use]
