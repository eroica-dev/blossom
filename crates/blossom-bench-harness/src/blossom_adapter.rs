...
pub struct BlossomTrustedPathSample {
    ...
    /// Immediate redb commits issued by the benchmark path for this write.
    ///
...
impl BlossomActiveActiveCluster {
    ...
    pub async fn start_with_holders_per_site(
        ...
        for (index, node) in order_cluster.cluster.nodes().iter().enumerate() {
            ...
            let store = DurableAdmissionStore::open(
                storage_root
                    .as_ref()
                    .join(format!("validator-{index}.redb")),
                site.clone(),
                store_generation,
                node.keypair.signer(),
            )?;
        ...
        let validators = order_cluster
            ...
            .collect::<BTreeSet<_>>();
        let engine = GlobalOrderedEngine::new_with_trust_mode(
            stores[0].clone(),
    ...
    pub async fn client_write(
        ...
        let phase_started = Instant::now();
        let (watermark, result) = match self.engine.apply_contiguous()? {
    ...
    pub fn read_local(&mut self, key: &[u8]) -> blossom::Result<Option<Vec<u8>>> {
        self.engine.read(key, blossom::ReadConsistency::Local, None)
    }
    ...
    pub async fn client_write_universal(
        ...
        let phase_started = Instant::now();
        let results = match self.engine.apply_through(last_watermark)? {
            ApplyProgress::Applied { watermark, results } if watermark == last_watermark => results,
...
mod tests {
    ...
    use blossom::{
        ActiveActiveCommand, AdmittedCommand, BatchReferenceMetadata, ClientEpoch, ClientId,
        CommandBatch, CommandIdentity, CommandOperation, ConsensusGroupId, Keypair,
        ReplicaMembershipEpoch, ValidatorGeneration,
    };
