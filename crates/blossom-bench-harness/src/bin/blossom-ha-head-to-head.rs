...
use blossom::{
    ActiveActiveCommand, ClientEpoch, ClientId, CommandIdentity, HaReplicationMode,
    HaServiceTopology, HashType, HighAvailabilityParameters, high_availability_fault_tolerance,
    high_availability_majority,
};
use blossom_bench_harness::{
    CommandOperation, CommandResult, FixedSlotHaCluster, HaAppliedSample, InProcessRaftCluster,
    OPENRAFT_VERSION, PairedRunOrder, RaftAppliedResponse, RaftStorageProfile,
    SHARD_STREAM_REVISION, active_active_command, paired_run_order,
};
use clap::Parser;
use serde::Serialize;
...
async fn main() -> Result<(), BoxError> {
    ...
    for physical_nodes in 2..=7 {
        ...
        let raft_storage = match &storage_root {
            Some(root) => RaftStorageProfile::DurableShardStream {
    ...
    let artifact = HaHeadToHeadArtifact {
        ...
        blossom_profile: if args.durable {
            "leaderless-active-active-fixed-slot-shard-stream"
        } else {
            "leaderless-active-active-fixed-slot-in-memory"
        },
        raft_profile: if args.durable {
            "leader-based-active-passive-in-process-shard-stream"
        } else {
