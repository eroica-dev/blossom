...
use blossom::{
    ...
    ConsensusGroupId, HaAcknowledge, HaConfirm, HaDispatch, HighAvailabilityParameters,
    HighAvailabilityRuntime, NodeIdentity, PubKey, Transaction, high_availability_fault_tolerance,
...
const HA_APPLICATION_TABLE: &str = "ha_benchmark_application_v1";
...
impl FixedSlotHaCluster {
    ...
    pub fn with_durable_storage(
        ...
        let runtimes = members
            ...
            .map(|(index, member)| {
                HighAvailabilityRuntime::open(
                    root.as_ref().join(format!("node-{index}")),
                    group,
                    member.public_key(),
                    members.clone(),
                    parameters,
        ...
        let applications = (0..node_count)
            .map(|index| {
                HaApplication::durable(
                    root.as_ref().join(format!("application-{index}")),
                    group,
                    index,
                )
            })
...
impl HaApplication {
    ...
    fn durable(
        ...
        let public_scope = borsh::to_vec(&(group_id, node_index as u64))?;
        let identity = BlossomLogStoreIdentity::new("benchmark-ha-application", public_scope, 1)?;
        let store = BlossomLogStore::open(BlossomLogStoreConfig::new(path.as_ref()), identity)?;
        let machine = store
            .get(HA_APPLICATION_TABLE, b"state")?
            .map(|bytes| borsh::from_slice(&bytes))
            .transpose()?
            .unwrap_or(SharedStateMachine::new(4096)?);
    ...
    fn persist(&self) -> Result<(), BoxError> {
        ...
        store.transaction(|transaction| {
            transaction.insert(HA_APPLICATION_TABLE, b"state".to_vec(), encoded)?;
            Ok(())
        })?;
        Ok(())
    }
