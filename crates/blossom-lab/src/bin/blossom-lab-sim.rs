...
fn write_bug_log(
    ...
    writeln!(file, "# Blossom Lab Bug Log")?;
    writeln!(file)?;
...
fn event_record_csv(record: &blossom_lab::HermeticEventRecord) -> String {
...
fn records_with(
    log: &HermeticEventLog,
    predicate: impl Fn(&blossom_lab::HermeticEventRecord) -> bool,
) -> Vec<&blossom_lab::HermeticEventRecord> {
    log.records
        .iter()
...
fn unavailable_by_target(
    log: &HermeticEventLog,
) -> BTreeMap<usize, Vec<&blossom_lab::HermeticEventRecord>> {
...
fn sample_event_ids(records: &[&blossom_lab::HermeticEventRecord]) -> String {
