...
pub async fn run_node_io_fuzz(cluster: &LabCluster, config: &FuzzConfig) -> Result<FuzzReport> {
...
mod tests {
    use super::*;
    use crate::chaos::{LabCluster, NetworkChaosConfig};

    #[tokio::test]
    async fn deterministic_fuzz_keeps_node_alive() {
        let cluster = LabCluster::spawn(1, NetworkChaosConfig::default())
            .await
            .unwrap();
