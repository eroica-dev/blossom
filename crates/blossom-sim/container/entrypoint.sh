#!/usr/bin/env bash
# Container entrypoint for the Blossom simulation tools.
#
...
run_fuzz() {
  local out="$results_dir/sim_fuzz_$(timestamp).csv"
  local expect_alive=()
...
run_sim() {
  local summary="$results_dir/sim_hermetic_$(timestamp).csv"
...
run_epoch_chaos() {
  ...
  local stages="${summary%.csv}_stages.csv"
  local bugs="${summary%.csv}_bugs.md"
  local trusted=()
  local shuffle=()
  ...
  if [[ -n "${OBSERVER_ADDR:-}" ]]; then
    observer=(--observer-addr "$OBSERVER_ADDR")
  fi

  blossom-sim-epoch-chaos \
    --nodes "${NODES:-36}" \
    --epochs "${EPOCHS:-4}" \
...
case "$mode" in
  ...
  all)
    ...
    ;;
  *)
    echo "unknown blossom-sim container mode: $mode" >&2
