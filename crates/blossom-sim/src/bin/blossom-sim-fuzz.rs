...
use blossom_sim::{DataPattern, FuzzConfig, FuzzReport, NetworkChaosConfig, SimTcpCluster};
use clap::Parser;
...
async fn main() -> MainResult<()> {
    ...
    let cluster = SimTcpCluster::spawn(args.nodes, NetworkChaosConfig::default()).await?;
    ...
    let report = blossom_sim::run_node_io_fuzz(&cluster, &config).await?;
