...
```toml
[dependencies]
blossom = { package = "blossom-consensus", version = "2", features = ["parallel-networks"] }
```
...
When the application changes routing during HA recovery, use
`ActiveActiveHaEngine` to produce the recovery and cutover manifests first.
Global `BatchReference` format v2 then commits the activated
`route_generation` and `command_spec_version`, preventing an old route or
decoder from being replayed under the new application contract.
